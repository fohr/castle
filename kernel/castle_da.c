                                         ...
                                         int is_re_add)
{
    ...
    debug("%s::[%p] Adding an entry at depth: %u for merge id %u level %u (out tree depth: %d)\n",
            __FUNCTION__, merge, depth, merge->id, merge->level, atomic_read(&merge->out_tree->tree_depth));
    ...
    if(new_root_node)
    {
        ...
        if (unlikely(atomic_read(&merge->out_tree->tree_depth) == -1))
            atomic_add(2, &merge->out_tree->tree_depth);
        else
            atomic_inc(&merge->out_tree->tree_depth);
...
static void castle_da_merge_serialise(struct castle_da_merge *merge, int using_tvr, int tvr_new_key)
{
    ...
    */
    ...
    if( unlikely(current_state == INVALID_DAM_SERDES) )
    {
        ...
        if( is_new_key )
        {
            /* Here we need something stronger than serdes mutex because we want to guarantee
               synchronization between merge serdes state, version stats, ct stats etc. */
            CASTLE_TRANSACTION_BEGIN;
    ...
    if( likely(current_state == VALID_AND_FRESH_DAM_SERDES) )
    {
        ...
        /* No-op: Wait for checkpoint to write existing serialisation point before updating it.
                                     ...
                                     c_da_merge_marshall_set_t partial_marshall)
{
    ...
    for(i=0; i<merge->nr_trees; i++)
    {
        ...
        /* There are 'completed' flags for each immut iterator within c_immut_iter_t and also
           ...
           states at serialisation boundary, which means we have to handle both separately. */
 ...
 */
static int castle_da_merge_trigger(struct castle_double_array *da, int level, int nr_trees)
{
    ...
    if ((da->levels[level].nr_trees - da->levels[level].nr_output_trees) < nr_trees)
        goto out;
...
static int castle_da_l1_merge_run(void *da_p)
{
    ...
    do {
        ...
        __wait_event_interruptible(da->merge_waitq,
                                   (ret = exit_cond) ||
                                        castle_da_merge_trigger(da, level, nr_trees),
                                   ignore);
        ...
        ret = castle_da_merge_cts_get(da, level, in_trees, nr_trees);
...
static int castle_da_merge_run(void *da_p)
{
    ...
    do {
        /* Wait for 2+ trees to appear at this level.
           NOTE: we moved exit condition from */
        __wait_event_interruptible(da->merge_waitq,
                    (ret = exit_cond) || castle_da_merge_trigger(da, level, 2),
                    ignore);
        ...
        /* Extract the two oldest component trees. */
        ret = castle_da_merge_cts_get(da, level, in_trees, 2);
        BUG_ON(ret && (ret != -EAGAIN));
        if(ret == -EAGAIN)
            goto __again;
        ...
        merge = castle_da_merge_alloc(2, level, da, INVAL_MERGE_ID, in_trees, nr_data_exts,
                                      data_exts);
        if (merge == NULL)
        {
            /* Merge failed, wait 10s to retry. */
...
static struct castle_double_array* castle_da_alloc(c_da_t da_id, c_da_opts_t opts)
{
    ...
    if (castle_golden_nugget)
    {
        castle_da_get(da);
        da->levels[1].merge.thread = kthread_create(castle_da_l1_merge_run,
                                                    da, "castle-m-%d-L1", da_id);
        if(IS_ERR(da->levels[1].merge.thread) || !da->levels[1].merge.thread)
        {
            castle_printk(LOG_WARN, "Failed to allocate memory for DA threads\n");
            da->levels[1].merge.thread = NULL;
            goto err_out;
        }
    }
    else
    {
        ...
        {
            castle_da_get(da);
            da->levels[i].merge.thread = kthread_create(castle_da_merge_run,
    ...
    while (castle_mstore_iterator_has_next(iterator))
    {
        ...
        if(!EXT_POS_INVAL(merge->serdes.mstore_entry->redirection_partition_node_cep))
        {
            ...
            merge->redirection_partition.node_c2b =
                castle_cache_block_get(merge->serdes.mstore_entry->redirection_partition_node_cep, node_size);
